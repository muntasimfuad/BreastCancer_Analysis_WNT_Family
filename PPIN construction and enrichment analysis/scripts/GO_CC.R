# GO Enrichment Analysis & Visualization: Cellular Component (CC) 
...
# Perform Gene Ontology (GO) enrichment analysis for Cellular Component (CC)
GO_CC <- enrichGO(
  genelist,
  OrgDb = "org.Hs.eg.db",
  keyType = "ENTREZID",
...
# Customize the plot
cc_pal <- c("#e34a33", "#fdbb84")

cc_vis <- cc_bar + scale_fill_gradientn(colors = cc_pal)+
  ...
  theme(axis.text.y = element_text(margin = margin(r = 5),
        ...
        panel.border = element_rect(colour = "#636363", 
                                    fill=NA,
                                    linewidth=1)) +
  ggtitle("Cellular Component (CC)")

# Save the plot
ggsave("PPIN construction and enrichment analysis/figures/CC.png", 
       plot = cc_vis, 
       width = 16, 
       height = 14, 
       dpi = 300,
...
cc_data <- cc_bar$data
write.csv(cc_data,"PPIN construction and enrichment analysis/outputs/GO_CC.csv")
