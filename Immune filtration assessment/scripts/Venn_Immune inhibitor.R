...
# Load data
inhibitor <- read_xlsx("Immune filtration assessment/data/Immune inhibitor correlation.xlsx")

# Define the gene lists
WNT2 <- inhibitor$WNT2
WNT7B <- inhibitor$WNT7B
WNT11 <- inhibitor$WNT11

# Create a list of the gene sets
gene_sets <- list(WNT2, WNT7B, WNT11)

# Generate the Venn diagram
inhibit_venn <- ggVennDiagram(gene_sets,
  ...
  scale_fill_gradientn(colors = c("#D6EBECFF", "#7AD8DFFF", "#72E1E1FF")) +
  labs(title = "Immune Inhibitors",
       subtitle = "Venn Diagram Visualizing Common Immune Inhibitors Across Three Genesets",
       fill = "immune inhibitors") +
  theme(legend.position = "right", 
...
# Save the plot
ggsave(filename = "Immune filtration assessment/figures/Immune inhibitors.png",
       plot = inhibit_venn +
